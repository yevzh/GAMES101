...
{   
    ...
    bool flag = AB.cross(AP).z() > 0 && BC.cross(BP).z() > 0 && CA.cross(CP).z() > 0 || AB.cross(AP).z() < 0 && BC.cross(BP).z() < 0 && CA.cross(CP).z() < 0;
    return flag;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    // first find the min and max x,y
   ...
   // ymax = (int)std::ceil(ymax);
    ...
    if (!MSAA)
    {
        ...
        // start iterating
        for (int x = (int)std::floor(xmin); x < (int)std::ceil(xmax); x++)
        {
            for (int y = (int)std::floor(ymin); y < (int)std::ceil(ymax); y++)
            {
                if (!insideTriangle(x, y, t.v))continue;
                // If so, use the following code to get the interpolated z value.
                auto ken = computeBarycentric2D(x, y, t.v);
                float alpha = std::get<0>(ken), beta = std::get<1>(ken), gamma = std::get<2>(ken);
    ...
    else
    {
        ...
        for (int x = (int)std::floor(xmin); x < (int)std::ceil(xmax); x++)
        {
            for (int y = (int)std::floor(ymin); y < (int)std::ceil(ymax); y++)
            {
                ...
                if (insideCnt)
                {
                    if (depth_buf[buf_index] > minDepth)
                    {
                        Vector3f color = t.getColor() * insideCnt / 4.0;
                        Vector3f point(3);
